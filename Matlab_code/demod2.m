function [demod_array]=demod2(data, fsymbol, fsample,preamble)
...
for i = 1:1:length(preamble)
    if preamble(i) == 1
        preamble_seq = [preamble_seq,ones(1,fsample/2/fsymbol),zeros(1,fsample/2/fsymbol)];
    else
...
while (loc_indx<ceil(0.9*length(new_data)))
    ...
    loc_indx = istop+fsample/fsymbol*16+11;
    ...
    if loc_indx <= count_sam * 2500
        if dist <= 15
            ...
            [payload_demod] = packet_demod (payload,signal,0.4,fsample/fsymbol,16);
